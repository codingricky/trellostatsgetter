...
describe DownloadedCardService do
  ...
  subject { DownloaderService.all('Sydney - Software Engineers') }
  ...
  context 'strip sensitive info' do
    it 'removes money values from the card name' do
      (DownloaderService.sanitize_name('70000').should eq(''))
      (DownloaderService.sanitize_name('70 000').should eq('70 '))
      (DownloaderService.sanitize_name('700000').should eq(''))
      (DownloaderService.sanitize_name('700 000').should eq(' '))
      (DownloaderService.sanitize_name('70,000').should eq(''))
      (DownloaderService.sanitize_name('70, 000').should eq(' '))
      (DownloaderService.sanitize_name('$70 000').should eq(' '))
      (DownloaderService.sanitize_name('$700000').should eq(''))
      (DownloaderService.sanitize_name('$700 000').should eq(' '))
      (DownloaderService.sanitize_name('$70,000').should eq(''))
      (DownloaderService.sanitize_name('$70, 000').should eq(' '))
      (DownloaderService.sanitize_name('70, 000 - 80, 000').should eq('  -  '))
      (DownloaderService.sanitize_name('70.000').should eq('70.'))
      (DownloaderService.sanitize_name('20k').should eq(''))
      (DownloaderService.sanitize_name('135k').should eq(''))
      (DownloaderService.sanitize_name('$20k').should eq(''))
      (DownloaderService.sanitize_name('$135k').should eq(''))
      (DownloaderService.sanitize_name('135k - 140k').should eq(' - '))
      (DownloaderService.sanitize_name('$135k - $140k').should eq(' - '))
      (DownloaderService.sanitize_name('145 per day').should eq(' per day'))
      (DownloaderService.sanitize_name('1200 per week').should eq(' per week'))
      (DownloaderService.sanitize_name('13/05/2016').should eq('13/05/'))
      (DownloaderService.sanitize_name('Billy Bob and Meggy Meg grabbed the $50 000 and ran').should eq('Billy Bob and Meggy Meg grabbed the   and ran'))
    end
  end
end
