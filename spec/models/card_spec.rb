...
describe Card do
  ...
  context 'strip sensitive info' do
    it 'removes money values from the card name' do
      (Card.new(name: '70000')).sanitized_name.should eq('')
      (Card.new(name: '70 000')).sanitized_name.should eq('70 ')
      (Card.new(name: '700000')).sanitized_name.should eq('')
      (Card.new(name: '700 000')).sanitized_name.should eq(' ')
      (Card.new(name: '70,000')).sanitized_name.should eq('')
      (Card.new(name: '70, 000')).sanitized_name.should eq(' ')
      (Card.new(name: '$70 000')).sanitized_name.should eq(' ')
      (Card.new(name: '$700000')).sanitized_name.should eq('')
      (Card.new(name: '$700 000')).sanitized_name.should eq(' ')
      (Card.new(name: '$70,000')).sanitized_name.should eq('')
      (Card.new(name: '$70, 000')).sanitized_name.should eq(' ')
      (Card.new(name: '70, 000 - 80, 000')).sanitized_name.should eq('  -  ')
      (Card.new(name: '70.000')).sanitized_name.should eq('70.')
      (Card.new(name: '20k')).sanitized_name.should eq('')
      (Card.new(name: '135k')).sanitized_name.should eq('')
      (Card.new(name: '$20k')).sanitized_name.should eq('')
      (Card.new(name: '$135k')).sanitized_name.should eq('')
      (Card.new(name: '135k - 140k')).sanitized_name.should eq(' - ')
      (Card.new(name: '$135k - $140k')).sanitized_name.should eq(' - ')
      (Card.new(name: '145 per day')).sanitized_name.should eq(' per day')
      (Card.new(name: '1200 per week')).sanitized_name.should eq(' per week')
      (Card.new(name: '13/05/2016')).sanitized_name.should eq('13/05/')
      (Card.new(name: 'Billy Bob and Meggy Meg grabbed the $50 000 and ran')).sanitized_name.should eq('Billy Bob and Meggy Meg grabbed the   and ran')
    end
  end
